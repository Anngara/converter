...
public interface InputOutput {
	...
	default Long getLong0(String prompt) {
		Long respounse = null;
		String initialPrompt = prompt;
		do {
			try {
				respounse = Long.parseLong(getString(prompt));
			} catch (NumberFormatException e) {
				// no operation
			}
			prompt = "Wrong input - see options" + initialPrompt;
	...
	default Double getDouble(String prompt) {
		Double respounse = null;
		String initialPrompt = prompt;
		do {
			try {
				respounse = Double.parseDouble(getString(prompt));
			} catch (Exception e) {
				respounse = null;
			}
			prompt = "Wrong input - see options" + initialPrompt;
	...
	// do a parameterized method.
	// why I dont make private method here
